...
const ProfilePage = () => {
  ...
  // Twitter Embed Component with error handling
  const TwitterEmbedWithFallback = ({ tweetUrl }: { tweetUrl: string }) => {
    ...
    if (loading) {
      return (
        <div className="flex justify-center items-center p-8 bg-gray-100 dark:bg-gray-800 rounded-lg min-h-[200px]">
    ...
    if (error) {
      return (
        <div className="p-4 border border-red-200 bg-red-50 dark:bg-red-900/20 dark:border-red-800/30 text-red-700 dark:text-red-400 rounded-lg flex items-center justify-center gap-2">
          <AlertCircle className="h-4 w-4" />
          <span>Failed to load tweet.</span>
          <a href={tweetUrl} target="_blank" rel="noopener noreferrer" className="underline font-medium">
            View on Twitter
          </a>
        </div>
      );
    ...
    return <div ref={containerRef} className="twitter-embed min-h-[100px] flex justify-center" />;
  };
  
  // LinkedIn Embed Component with fallback
  const LinkedInEmbedWithFallback = ({ linkedInUrl }: { linkedInUrl: string }) => {
    const containerRef = useRef<HTMLDivElement>(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(false);
    ...
    useEffect(() => {
      ...
      try {
        ...
        script.async = true;
        script.onload = () => {
          if (window.IN && containerRef.current) {
            // Create a container for the LinkedIn post
            const embedContainer = document.createElement('div');
            embedContainer.className = 'linkedin-post-embed';
            containerRef.current.appendChild(embedContainer);
            
            // Create the LinkedIn embed
            const postDiv = document.createElement('div');
            postDiv.className = 'linkedin-embedded-post';
            postDiv.setAttribute('data-url', linkedInUrl);
            embedContainer.appendChild(postDiv);
            
            // Try to render the LinkedIn post
            try {
              window.IN.parse();
              setLoading(false);
            } catch (parseError) {
              console.error('LinkedIn parse error:', parseError);
              setLoading(false);
              setError(true);
            }
          } else {
        ...
        // Fallback timeout in case LinkedIn embed takes too long
        ...
        console.error('Error embedding LinkedIn post:', err);
        setLoading(false);
        setError(true);
      }
    }, [linkedInUrl, loading]);
    ...
    if (loading) {
      return (
        <div className="flex justify-center items-center p-8 bg-gray-100 dark:bg-gray-800 rounded-lg min-h-[200px]">
          <div className="h-6 w-6 rounded-full border-2 border-primary border-r-transparent animate-spin"></div>
          <span className="ml-2 text-sm text-muted-foreground">Loading LinkedIn post...</span>
        </div>
      );
    }
    ...
    if (error) {
      return (
        <div className="p-4 border border-blue-200 bg-blue-50 dark:bg-blue-900/20 dark:border-blue-800/30 text-blue-700 dark:text-blue-400 rounded-lg flex items-center justify-center gap-2">
          <AlertCircle className="h-4 w-4" />
          <span>Failed to load LinkedIn post.</span>
    ...
    return <div ref={containerRef} className="linkedin-embed min-h-[100px] flex justify-center" />;
  };

  // Component to render the appropriate embed based on platform
  const ProofOfWorkItem = ({ pow, index }: { pow: any; index: number }) => {
    return (
      <motion.div
          ...
          <div className="mb-4 flex justify-center">
            {pow.platform === "twitter" && (
              <div style={{ width: '100%', maxWidth: '550px' }}>
                <TwitterEmbedWithFallback tweetUrl={pow.postUrl} />
              </div>
            )}
            {pow.platform === "linkedin" && (
              <div style={{ width: '100%', maxWidth: '550px' }}>
                <LinkedInEmbedWithFallback linkedInUrl={pow.postUrl} />
              </div>
            )}
            {pow.platform === "github" && (
                      ...
                      <TabsContent value="pow" className="space-y-6 mt-0">
                        <motion.div variants={itemVariants} className="space-y-4">
                          ...
                          <div className="mt-6 space-y-4">
                            ...
                            {user.proofOfWork?.map((pow: any, index: number) => (
                              <ProofOfWorkItem key={index} pow={pow} index={index} />
                            ))}
